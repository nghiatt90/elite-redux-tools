...
export default function PokedexList() {
  ...
  const results = useMemo(() => searchSpecies(searchIndex, query), [searchIndex, query])
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      const isTypingElsewhere = target !== inputRef.current && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA')
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-3 py-2 shrink-0 flex items-center gap-2" style={{ borderColor: 'var(--color-border)' }}>
        <input
          ref={inputRef}
          type="text"
          value={query}
      ...
      <div className="flex-1 min-h-0">
        {results.length === 0 ? (
          <div className="p-4 text-sm" style={{ color: 'var(--color-text-muted)' }}>
            No matches for "{query}" — try a shorter or different name.
          </div>
        ) : (
