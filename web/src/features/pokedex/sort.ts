...
export interface SortState {
  key: SortKey | null // null = unsorted -- keep whatever order the caller already produced
...
/** Applies an explicit sort on top of whatever order `species` already came in
...
export function applySort(species: Species[], sort: SortState, speciesById: Map<string, Species>): Species[] {
  ...
  return species
    .map((s, index) => ({ s, index }))
    .sort((a, b) => {
      const av = sortValue(a.s, key, speciesById)
      const bv = sortValue(b.s, key, speciesById)
